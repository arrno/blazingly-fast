...
export function Footer() {
  return (
    <footer className="relative overflow-hidden border-t border-white/10 bg-gray-950 text-gray-200">
      ...
      <PageSection
        className="relative flex flex-col gap-12 py-20 sm:py-24"
      >
        <div className="space-y-4 max-w-3xl">
          ...
          <p className="text-sm leading-relaxed text-white/70">
            Certified Blazingly Fast is a self-reported performance standard. We
            celebrate ambition, encourage optimism, and remind the world that
            benchmarks aren’t the only metric that matters.
          </p>
        ...
        <div className="flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/5 p-6 text-sm text-white/80 shadow-[0_20px_60px_-20px_rgba(15,23,42,0.6)] sm:flex-row sm:items-center sm:justify-between">
          ...
          <Link
            href="#submit"
            className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold text-gray-900 shadow-sm transition hover:bg-white/80"
          >
            Get certified
        ...
        <div className="flex flex-col gap-2 text-xs uppercase tracking-[0.25em] text-white/40 sm:flex-row sm:items-center sm:justify-between">
          <span>© {new Date().getFullYear()} blazingly.fast — run responsibly.</span>
